...
try {
    node tests/render-triage.cjs --polygons
    ...
    $polygons = Get-Content -LiteralPath 'docs/triage-preview-polygons.json' -Raw | ConvertFrom-Json
    ...
    $graphics.DrawString('TRIAGE / REFERENCE MODEL STUDY',$font,[System.Drawing.Brushes]::Ivory,32,20)
    $graphics.DrawString('REST',$font,[System.Drawing.Brushes]::Ivory,100,510)
    $graphics.DrawString('SANCTUARY',$font,[System.Drawing.Brushes]::Ivory,520,510)
    $graphics.DrawString('REAR',$font,[System.Drawing.Brushes]::Ivory,920,510)
    $canvas.Save((Join-Path (Get-Location) 'docs/triage-design.png'))
    $graphics.Dispose()
    $canvas.Dispose()
    $font.Dispose()
    Remove-Item -LiteralPath 'docs/triage-preview-polygons.json'
