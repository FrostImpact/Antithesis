...
function createHost(){
 ...
 const h={obj_game:{paused:false,build_tower_type:'vestral'},obj_tower:'tower',obj_placement:{exists:false,moving_tower:null},
  noone:null,min:Math.min,max:Math.max,abs:Math.abs,sin:Math.sin,exp:Math.exp,power:Math.pow,pi:Math.PI,
  ...
  make_colour_rgb:(...v)=>v,GlossaryTerm:{None:-1,GreatPowers:0,Charge:1,Lock:2},TowerAbility:{DoubleTap:0,ShockBolts:1,Overloaded:2},
  draw_defender:()=>{},defender_muzzle:()=>{},draw_wanderer:()=>{},wanderer_muzzle:()=>{},
 ...
 const api=new Function('s',`with(s){${read('scripts/scr_definitions/scr_definitions.gml')}\n${read('scripts/scr_loadout/scr_loadout.gml')};obj_game.tower_catalog=build_tower_catalog();obj_game.ui_theme=build_ui_theme();return {init:loadout_initialize,use:loadout_use_card,tick:loadout_tick,reward:loadout_round_reward,kill:loadout_award_kill,select:loadout_select,place:loadout_place,canPlace:loadout_can_place,blocked:loadout_pointer_blocked,click:loadout_handle_input,pose:loadout_card_pose,cardAt:loadout_card_at_pointer,draw:loadout_draw};}`)(h);
 api.init();return {h,api,towers};
}
function test(){
 const {h,api,towers}=createHost();const s=h.obj_game.loadout;
 assert.deepEqual(s.cards,[2,1,1,0]);assert.ok(s.keys.every(k=>k===''));
 ...
 api.use(1);api.tick(1);assert.equal(s.keys[1],'wanderer');s.bits=119;
 assert.equal(api.select(1),false);assert.equal(api.place(4,4),null);
 s.bits=120;api.select(1);assert.ok(api.place(4,4));assert.equal(s.bits,0);
 ...
 for(let r=1;r<=3;r++)assert.equal(api.reward(r),true);
 assert.deepEqual(s.cards.map((v,i)=>v-before[i]),[3,3,2,1],'A shuffled deck preserves its tower/item/gameplay composition');
 ...
 s.bits=1000;s.copies[0]=10;api.select(0);
 while(towers.length<6)assert.ok(api.place(2,2),'Five hotbar slots must not impose a five-deployed-tower limit');
 ...
 h.mouseX=400;h.mouseY=700;assert.equal(api.blocked(),true);
