...
const chargeDuration=Number(fs.readFileSync(path.join(root,'scripts/scr_definitions/scr_definitions.gml'),'utf8').match(/wanderer:\{[\s\S]*?charge_duration:([\d.]+)/)[1]);
...
obj_camera:{zoom:6},TowerChargeState:{Ready:0,Charging:1},tower_charge_progress:t=>1-t.charge_left/${chargeDuration},
...
const fireAt=1.5+${chargeDuration},t=clock%(fireAt+3),charging=t>=1.5&&t<fireAt,shot=t>=fireAt&&t<fireAt+.65,p=charging?(t-1.5)/${chargeDuration}:0;
const aim=charging?1-Math.exp(-3.5*(t-1.5)):shot?1:t>=fireAt+.65?Math.exp(-2.8*(t-fireAt-.65)):0;
const charge=charging?1-Math.exp(-9*(t-1.5)):0,kick=t>=fireAt?29*Math.exp(-8*(t-fireAt)):0;
...
const tower={x:420,y:490,idle_time:clock,facing:320,recoil:kick,aim_blend:aim,charge_pose:charge,charge_mode:charging?1:0,charge_left:${chargeDuration}*(1-p),finisher_flash:shot?.65-(t-fireAt):0,beam_world_x:960,beam_world_y:425,definition:{muzzle:api.muzzle}};
api.charge(tower);api.shot(tower);
document.getElementById('stage').textContent=charging?'EXECUTION / CHARGING':shot?'EXECUTION / FIRING':t>fireAt+.65&&t<fireAt+1.5?'SETTLING':'VIGIL / KNEELING';requestAnimationFrame(frame);}
